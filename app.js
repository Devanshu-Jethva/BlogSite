...
const MyModel = mongoose.model("Blog", MySchema);
...
app.get("/", (req, res) => {
  MyModel.find({}, (e, blog) => {
    res.render("home", { hsc: homeStartingContent, homePosts: blog });
  })
});

app.get("/posts/:blogId", (req, res) => {
  MyModel.find({ _id: req.params.blogId }, (e, blog) => {
    if (!e) {
      // console.log(blog[0].title);
      res.render("post", { pt: blog[0].title, pb: blog[0].body })
    }
    else {
      console.log(e);
    }
